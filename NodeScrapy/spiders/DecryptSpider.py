...
class DecryptSpider(SimpleSpider):
    ...
    def parse_blog(self, response: Response):
        ...
        old_date = datetime.datetime.strptime(self.configs[name].get("up_date", ""), "%Y-%m-%d").date() if self.configs[name].get("up_date") else None
        web_date = datetime.datetime.strptime(response.meta["date"], "%Y-%m-%d").date()
        method = {k: self.configs[name].get(k, "") for k in ["script", "textbox", "button"]}
        
        for pwd in itertools.chain([old_pwd], pwdfinder.password_iter()):
            if not pwd:
                continue
        ...
        if old_pwd and pwd and old_pwd != pwd and web_date > old_date:
